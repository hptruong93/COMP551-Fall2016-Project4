...
def save_model_parameters_theano(outfile, model):
    ...
    np.savez(outfile, U=U, V=V, W=W, lr = model.learning_rate)
    print "Saved model parameters to %s." % outfile
   
def load_model_parameters_theano(path, model):
    npzfile = np.load(path)
    U, V, W, lr = npzfile["U"], npzfile["V"], npzfile["W"], npzfile["lr"]
...
def accuracy_in_month(model,X,Y,t):
    count = 0
    total = 0
...
def train_with_sgd(model, X_train, y_train, learning_rate=0.005, nepoch=1, evaluate_loss_after=5):
    ...
    for epoch in range(nepoch):
        ...
        if (epoch+1 % evaluate_loss_after == 0):
            ...
            print "%s: Loss and train accuracy after num_examples_seen=%d epoch=%d: %f,%f" % (time, num_examples_seen, epoch, loss,acc)
...
def set_model_parameters(model,U=None, V=None, W=None, learning_rate = 0.005):
